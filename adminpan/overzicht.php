        ...
        <div class="col-12">
				<div id="content-box" style="max-height:100%">
					<div class="title-box png20" style="background-color:#00796B;background-image:url(/templates/david_vox/img/shouts.gif);background-repeat: no-repeat;background-position: right;height: 80px;">
						<div class="title"><font color="white">Yada HK</font></div>
...
<?php
			$getRanks = $dbh->prepare("SELECT id,rank_name,badge FROM permissions WHERE id in (9,8,7,6,5,4,3)  ORDER BY id DESC");
			$getRanks->execute();
			...
			{	
				...
				if ($getMembers->RowCount() > 0)
				{
					while ($member = $getMembers->fetch())
					{
						...
						$functie = filter($Ranks['rank_name']);
						...
						echo '
										...
										<td style="height: 60px;width: 70%;">' . $functie . '</td>
										<td style="width: 40%;"><img src="/templates/flunno/style/images/cms/'.$OnlineStatus.'.png"></td>
										</tr>
