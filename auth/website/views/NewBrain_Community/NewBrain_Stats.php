...
?>
    <title><?= $config['hotelName'] ?>: Stats</title>
</head>
...
<img src="<?= $config['hotelUrl'] ?>/auth/assets/img/" style="float:right;">
<hr>
<?php } ?>
        ...
        <div class="col-3">
				<div id="shadow-box" style="max-height:300px">
					...
					<div class="png20">
								<div class="event">
<?php 
$belcr_get = $dbh->prepare("SELECT pixels,username from users WHERE rank < 12 ORDER BY `pixels` DESC  LIMIT 5");
$belcr_get->execute();
while ($belcr_row = $belcr_get->fetch())
{
?>
<a href="/home/<?= filter($belcr_row['username']) ?>" style="text-decoration:none"><font color="grey"><?= filter($belcr_row['username']) ?></font></a>
<strong style="float:right;">&nbsp;&nbsp;<?= filter($belcr_row['pixels']) ?></strong>
<img src="<?= $config['hotelUrl'] ?>/auth/assets/img/ducket.png" style="float:right;">
<hr>
<?php } ?>
									<div style="clear: both"></div>
								</div>
        ...
        <div class="col-3">
				<div id="shadow-box" style="max-height:300px">
					...
					<div class="png20">
								<div class="event">
<?php 
$belcr_get = $dbh->prepare("SELECT points,username from users WHERE rank < 12 ORDER BY `points` DESC  LIMIT 5");
$belcr_get->execute();
while ($belcr_row = $belcr_get->fetch())
{
?>
<a href="/home/<?= filter($belcr_row['username']) ?>" style="text-decoration:none"><font color="grey"><?= filter($belcr_row['username']) ?></font></a>
<strong style="float:right;">&nbsp;&nbsp;<?= filter($belcr_row['points']) ?></strong>
